...
while true; do # Main production loop
    while true; do # Inner loop for error recovery
        ...
        while true; do
            ...
            openocd -f interface/stlink.cfg -c "transport select swd" -f target/stm32f0x.cfg -c "adapter speed 400; init; targets; exit" >detect.log 2>&1
        ...
        openocd -f interface/stlink.cfg -c "transport select swd" -f target/stm32f0x.cfg -c "adapter speed 400; init; reset halt; stm32f0x unlock 0; exit" > unlock.log 2>&1
        
        printf "${MAG}Start flashing STM32F030 with GameKit firmware${DEF}\n"
        openocd -f interface/stlink.cfg -c "transport select swd" -f target/stm32f0x.cfg -c "adapter speed 400; init; reset halt; flash write_image erase $APPLICATION_FW; verify_image $APPLICATION_FW; reset run; exit" > flash.log 2>&1
